...
export default class PlannerMermaid {
  ...
  private generateEntries(items: PlanItem[]): {
    tasks: string[];
    breaks: string[];
  } {
    const tasks: string[] = [];
    ...
    items.forEach((item, i) => {
      ...
      const text = `    ${this.escape(item.text)}    :${item.rawTime}, ${mins}`;
      if (item.isBreak) {
        breaks.push(text);
      } else {
        tasks.push(text);
  ...
  private minuteInterval(item: PlanItem, next: PlanItem): string {
    ...
    const untilNext = Math.floor(
      moment.duration(nextMoment.diff(currentMoment)).asMinutes()
    );
    return untilNext + "mm";
  }
  ...
  private escape(text: string) {
    ...
    mermaidEscapedCharacters.forEach((mec) => {
      ...
      ret = ret.replace(regex, mec.replacement);
    });
    return ret;
  }
